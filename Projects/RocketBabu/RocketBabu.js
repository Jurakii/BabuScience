...
function reset() {
  ...
  select = new sound("../Projects/RocketBabu/blipSelect.wav");
  select.play();
  dead = false;
...
function player() {
  drawing = new Image();
  if (velocity > 0) {
    drawing.src = "../Projects/RocketBabu/BabuFire.png";
  } else {
    drawing.src = "../Projects/RocketBabu/BabuFall.png";
  }
...
function died() {
  dead = true;
  explosion = new sound("../Projects/RocketBabu/explosion.wav");
  explosion.play();
  highScore(score);
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  background();
  ...
  if (start == false) {
    if (!dead) {
      ...
      if (scored == false && (((playerX + h > pipeX1) && (playerX + h < pipeX1 + 150)) || ((playerX + h > pipeX2) &&
        ...
        scored = true;
        score += 1;
        scoreSound = new sound("../Projects/RocketBabu/pickupCoin.wav");
        scoreSound.play();
      ...
      if (spacePressed) {
        velocity = 8;
        spacePressed = false;
        fly = new sound("../Projects/RocketBabu/jump.wav");
        fly.play();
      }
